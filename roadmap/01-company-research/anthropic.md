# Anthropic - Company Profile

## Overview

| Attribute | Details |
|-----------|---------|
| **Founded** | 2021 |
| **Structure** | Public Benefit Corporation (PBC) |
| **HQ** | San Francisco (also NYC, Seattle) |
| **Products** | Claude, Claude Code, Claude API |
| **Mission** | "Build AI systems that are reliable, interpretable, and steerable" |

## Company Culture

- **AI Safety First:** Primary emphasis on safety and interpretability
- **High-Trust Environment:** Low politics, transparent culture
- **Research + Engineering:** "Engineers do research, researchers do engineering"
- **Retention:** 80% employee retention rate (industry-leading)
- **Long-Term Benefit Trust:** Unique structure ensuring mission endurance

### Culture Quote
> "High-trust, low-politics environment where transparency is fostered and employees feel empowered to voice concerns directly to leadership."

## Key Technical Requirements

### Must-Have Skills (Senior Roles)
- **6+ years** software engineering experience
- **Strong Python** - Primary language for interviews
- **Service-oriented architectures** - Microservices, APIs
- **Product-focused mindset** - Robust, scalable, easy to use

### Work Location
- Minimum **25% in-office** (SF, NYC, or Seattle)
- Visa sponsorship available

## What Makes Anthropic Different

> "About half our technical staff had no prior ML experience; about half have PhDs, but plenty of brilliant colleagues never went to college."

> "If you've done interesting independent research, written a thoughtful blog post, or contributed to open source, put that at the TOP of your resume."

**Key Insight:** Direct evidence of ability matters more than credentials.

## Technical Focus Areas

1. **Constitutional AI** - Safety through AI feedback
2. **Interpretability** - Understanding model internals
3. **Claude Development** - Model training, fine-tuning
4. **Infrastructure** - Serving, scaling, reliability
5. **Safety Research** - Red-teaming, evaluation

## Compensation

| Level | Base Salary | Total Comp |
|-------|-------------|------------|
| Software Engineer | $280K-$400K | + equity matching |
| Senior ML Engineer | $400K-$485K | $470K-$630K median |
| Staff+ | $500K+ | Can exceed $900K |

*Equity donation matching, generous PTO, parental leave, flexible hours.*

## Recent Engineering Focus (2025)

Based on [Anthropic Engineering Blog](https://www.anthropic.com/engineering):

- **Multi-Agent Systems** - Claude's research capabilities with parallel agents
- **Context Engineering** - Building effective AI agents
- **Claude Code** - AI-powered development tools
- **Interpretability** - Transformer circuits research

## Research & Safety Focus

| Resource | Focus |
|----------|-------|
| [Alignment Science Blog](https://alignment.anthropic.com/) | Safety research updates |
| [Transformer Circuits](https://transformer-circuits.pub/) | Interpretability research |
| Sabotage Risk Reports | Model deployment safety |

## Resources

| Resource | Link |
|----------|------|
| Careers | [anthropic.com/careers](https://www.anthropic.com/careers) |
| Engineering Blog | [anthropic.com/engineering](https://www.anthropic.com/engineering) |
| Alignment Blog | [alignment.anthropic.com](https://alignment.anthropic.com/) |
| Research | [anthropic.com/research](https://www.anthropic.com/research) |

## Interview Tips Specific to Anthropic

1. **AI Safety is woven into every round** - Show you care about implications
2. **Speed and correctness over Big O** - They care about working code fast
3. **Prepare for progressive coding challenges** - 4 levels, increasing complexity
4. **No hidden test cases** - Interviewee-friendly format
5. **Read Alignment Science Blog** - Discuss Constitutional AI, interpretability
6. **Independent work matters** - Blog posts, open source, side projects

## Sample Interview Questions

### Coding
- Build an in-memory database: SET/GET/DELETE → filtered scans → TTL → compression
- Implement bank application with multiple transaction types
- LRU cache with *args and **kwargs handling

### System Design
- "Design a distributed search system for billion documents at million QPS, with LLM inference at 10K+ RPS"
- "Scale a token-generating function to handle 100,000 requests per second"

### Behavioral
- "Why is AI safety important to you personally?"
- "Describe a time you prioritized doing the right thing over the easy thing"
